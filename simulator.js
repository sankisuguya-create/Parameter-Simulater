...
export const defaults={oniCount:6,runnerCount:18,guardCount:1,mochiCount:0,oniLevel:5,oniVariantPercent:30,oniVariantLevel:7,runnerLevel:5,runnerVariantPercent:30,runnerVariantLevel:7,evaderPercent:20,oniAvoiderPercent:20,rescuerPercent:20,fieldWidth:70,fieldHeight:40,jailPosition:'right',jailWidth:8,jailHeight:6,restrictedEnabled:true,restrictedMode:'inside',restrictedX:23,restrictedY:11,restrictedWidth:24,restrictedHeight:18,captureTarget:18,timeLimit:180,oniStartDelay:10,touchRescue:true,touchAmount:'one',symbolRescue:false,symbolAmount:'one',symbolHold:2,symbolCooldown:8,protectionTime:4,rescueTendency:55,dangerDistance:5,guardRange:8,retargetDistance:18,nearbySwitchDistance:2.5,mochiLimit:30,sprintDuration:10,recoveryDuration:20};
...
export function validateConfig(c){
 ...
 const typeTotal=c.evaderPercent+c.oniAvoiderPercent+c.rescuerPercent;
...
export class Simulation{
 ...
 runnerTypeMap(start,count){const c=this.config,groups=[['evader',c.evaderPercent],['oniAvoider',c.oniAvoiderPercent],['rescuer',c.rescuerPercent]],used=groups.reduce((n,[,p])=>n+p,0);groups.push(['standard',100-used]);const allocations=groups.map(([type,p],index)=>({type,index,count:Math.floor(count*p/100),fraction:count*p/100-Math.floor(count*p/100)})),remaining=count-allocations.reduce((n,g)=>n+g.count,0);allocations.sort((a,b)=>b.fraction-a.fraction||a.index-b.index);for(let i=0;i<remaining;i++)allocations[i%allocations.length].count++;allocations.sort((a,b)=>a.index-b.index);const ranked=Array.from({length:count},(_,i)=>{const id=start+i;return{id,score:hash(this.seed^0xa17e5,id)()}}).sort((a,b)=>a.score-b.score||a.id-b.id),result=new Map();let cursor=0;for(const group of allocations)for(let i=0;i<group.count;i++)result.set(ranked[cursor++].id,group.type);return result}
